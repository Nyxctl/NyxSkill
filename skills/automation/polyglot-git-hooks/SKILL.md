...
description: "Design fast, portable Git hooks that enforce useful local quality checks without duplicating the full CI pipeline."
id: nyx.automation.polyglot-git-hooks
version: 1.0.0
---
# Polyglot Git Hooks

## Purpose
Design fast, portable Git hooks that enforce useful local quality checks without duplicating the full CI pipeline.

## When to Use
Use when the request matches: git hooks, pre-commit, pre push.
...
2. Apply this skill's focus: Design fast, portable Git hooks that enforce useful local quality checks without duplicating the full CI pipeline.
